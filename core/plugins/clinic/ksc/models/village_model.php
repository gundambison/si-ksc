<?php
class Models_Clinic_Ksc_Village_model extends Base_Model {
var $table='klinik_village'; //main table
/*
	Detail Village berdasarkan id
*/
	public function detail($id,$field='vil_id')
	{
		$sql=sprintf("select 
		`vil_id`, `vil_name`, `vil_code`, `vil_stat`
		from 
		`{$this->table}` 
		where `%s`='%s'",$field, $id);
		$data=$this->resOne($sql,1);
		if(count($data)>1){
			$row=$this->cleanFieldName('vil_',$data);
			return $row;
		}
		else{
			return false;
...
*/	
	...
	{
		$sql=sprintf("select count(vil_id) c from `%s`",
		$this->table);
...
/*
	Insert data 
	list field tersedia : `vil_id`, `vil_name`, `vil_code`, `vil_stat` 
	id : vil_id
*/
	public function add($data){
		$id=$this->idTable('vil_',716352);
...
*/
	public function __construct(){
		...
		$this->logger->write('info', 'model: Village');
	}
