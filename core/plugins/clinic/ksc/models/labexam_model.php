<?php
class Models_Clinic_Ksc_Labexam_model extends Base_Model {
var $table='klinik_labexam'; //main table
/*
	Detail Labexam berdasarkan id
*/
	public function detail($id,$field='ex_id')
	{
		$sql=sprintf("select 
		`ex_id`, `ex_name`, `ex_sub`, `ex_normal`, `ex_stat`, `ex_cat`
		from 
		`{$this->table}` 
		where `%s`='%s'",$field, $id);
		$data=$this->resOne($sql,1);
		if(count($data)>1){
			$row=$this->cleanFieldName('ex_',$data);
			return $row;
		}
		else{
			return false;
...
*/	
	...
	{
		$sql=sprintf("select count(ex_id) c from `%s`",
		$this->table);
...
/*
	Insert data 
	list field tersedia : `ex_id`, `ex_name`, `ex_sub`, `ex_normal`, `ex_stat`, `ex_cat` 
	id : ex_id
*/
	public function add($data){
		$id=$this->idTable('ex_',716352);
...
*/
	public function __construct(){
		...
		$this->logger->write('info', 'model: Labexam');
	}
