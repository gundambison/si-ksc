<?php
class Models_Clinic_Ksc_Pay_model extends Base_Model {
var $table='klinik_payby'; //main table
/*
	Detail Pay berdasarkan id
*/
	public function detail($id,$field='pay_id')
	{
		$sql=sprintf("select 
		`pay_id`, `pay_name`
		from 
		...
		if(count($data)>1){
			$row=$this->cleanFieldName('pay_',$data);
			return $row;
		}
		else{
			return false;
...
*/	
	...
	{
		$sql=sprintf("select count(pay_id) c from `%s`",
		$this->table);
...
*/
	public function __construct(){
		...
		$this->logger->write('info', 'model: Pay');
	}
