...
/*
	...
	access	: Clinic/ksc/village
	
	list field tersedia : 
			  `vil_id`, `vil_name`, `vil_code`, `vil_stat` 
	id 		: vil_id
*/
class Controllers_Clinic_Ksc_Village extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('village_model'); //base
...
*/
		$village=$this->model->getAll();
		$result['show']=count($village);
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `vil_id`, `vil_name`, `vil_code`, `vil_stat` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
