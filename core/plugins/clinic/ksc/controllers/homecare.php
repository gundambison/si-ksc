...
/*
	...
	access	: Clinic/ksc/homecare
	
	list field tersedia : 
			  `daf_id`, `daf_daf`, `daf_pat`, `daf_doc`, `daf_pay`, `daf_date`, `daf_user`, `daf_stat` 
	id 		: daf_id
*/
class Controllers_Clinic_Ksc_Homecare extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('homecare_model'); //base
...
*/
		$homecare=$this->model->getAll();
		$result['show']=count($homecare);
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `daf_id`, `daf_daf`, `daf_pat`, `daf_doc`, `daf_pay`, `daf_date`, `daf_user`, `daf_stat` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
