...
/*
	...
	access	: Clinic/ksc/labexam
	
	list field tersedia : 
			  `ex_id`, `ex_name`, `ex_sub`, `ex_normal`, `ex_stat`, `ex_cat` 
	id 		: ex_id
*/
class Controllers_Clinic_Ksc_Labexam extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('labexam_model'); //base
...
*/
		$labexam=$this->model->getAll();
		$result['show']=count($labexam);
		$result['labexam']=$labexam;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `ex_id`, `ex_name`, `ex_sub`, `ex_normal`, `ex_stat`, `ex_cat` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
