...
/*
	...
	access	: Clinic/ksc/homecareitem
	
	list field tersedia : 
			  `item_id`, `item_name` 
	id 		: item_id
*/
class Controllers_Clinic_Ksc_Homecareitem extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('homecareitem_model'); //base
...
/*		
	list field tersedia : `item_id`, `item_name` 	
*/
		$homecareitem=$this->model->getAll();
		$result['show']=count($homecareitem);
		$result['homecareitem']=$homecareitem;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `item_id`, `item_name` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
