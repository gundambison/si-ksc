...
/*
	...
	access	: Clinic/ksc/agama
	
	list field tersedia : 
			  `religi_id`, `religi_name` 
	id 		: religi_id
*/
class Controllers_Clinic_Ksc_Agama extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('agama_model'); //base
...
*/
		$agama=$this->model->getAll();
		$result['show']=count($agama);
		$result['agama']=$agama;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `religi_id`, `religi_name` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
