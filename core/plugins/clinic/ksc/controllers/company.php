...
/*
	...
	access	: Clinic/ksc/company
	
	list field tersedia : 
			  `com_id`, `com_name`, `com_det`, `com_stat` 
	id 		: com_id
*/
class Controllers_Clinic_Ksc_Company extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('company_model'); //base
...
*/
		$company=$this->model->getAll();
		$result['show']=count($company);
		$result['company']=$company;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `com_id`, `com_name`, `com_det`, `com_stat` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
