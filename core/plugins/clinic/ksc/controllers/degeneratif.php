...
/*
	...
	access	: Clinic/ksc/degeneratif
	
	list field tersedia : 
			  `deg_id`, `deg_name` 
	id 		: deg_id
*/
class Controllers_Clinic_Ksc_Degeneratif extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('degeneratif_model'); //base
...
*/
		$degeneratif=$this->model->getAll();
		$result['show']=count($degeneratif);
		$result['degeneratif']=$degeneratif;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `deg_id`, `deg_name` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
