<?php
/*
	Name	: Labexamcategory
	Created : 2015-02-13 11:18:40
	By		: Gunawan Wibisono (gundambison@gmail.com)
	access	: Clinic/ksc/labexamcategory
	
	list field tersedia : 
			  `excat_id`, `excat_name` 
	id 		: excat_id
*/
class Controllers_Clinic_Ksc_Labexamcategory extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('labexamcategory_model'); //base
...
*/
		$labexamcategory=$this->model->getAll();
		$result['show']=count($labexamcategory);
		$result['labexamcategory']=$labexamcategory;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `excat_id`, `excat_name` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
