...
/*
	...
	access	: Clinic/ksc/asuransi
	
	list field tersedia : 
			  `ins_id`, `ins_name`, `ins_detail`, `ins_stat` 
	id 		: ins_id
*/
class Controllers_Clinic_Ksc_Asuransi extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('asuransi_model'); //base
...
*/
		$asuransi=$this->model->getAll();
		$result['show']=count($asuransi);
		$result['asuransi']=$asuransi;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `ins_id`, `ins_name`, `ins_detail`, `ins_stat` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
