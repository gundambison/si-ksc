<?php
/*
	Name	: Rawatinapbangsal
	Created : 2015-02-13 11:18:40
	By		: Gunawan Wibisono (gundambison@gmail.com)
	access	: Clinic/ksc/rawatinapbangsal
	
	list field tersedia : 
			  `bang_id`, `bang_name`, `bang_sec` 
	id 		: bang_id
*/
class Controllers_Clinic_Ksc_Rawatinapbangsal extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('rawatinapbangsal_model'); //base
...
*/
		$rawatinapbangsal=$this->model->getAll();
		$result['show']=count($rawatinapbangsal);
		$result['rawatinapbangsal']=$rawatinapbangsal;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `bang_id`, `bang_name`, `bang_sec` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
