...
/*
	...
	access	: Clinic/ksc/rawatinapitem
	
	list field tersedia : 
			  `item_id`, `item_code`, `item_name`, `item_type`, `item_time` 
	id 		: item_id
*/
class Controllers_Clinic_Ksc_Rawatinapitem extends Modules_Plugin_Base {
...
//		$this->logger->write('debug', 'HTTP REQUEST: '.print_r($_REQUEST,1));
		$this->model = $this->_loadModel('rawatinapitem_model'); //base
...
*/
		$rawatinapitem=$this->model->getAll();
		$result['show']=count($rawatinapitem);
		$result['rawatinapitem']=$rawatinapitem;
...
//============tambah data bila diperlukan
		...
		$data['status']=1;
/*		
	list field di input : `item_id`, `item_code`, `item_name`, `item_type`, `item_time` 	
*/		
		if($this->add($data))
			$this->_success();
		$this->_failed(209);
